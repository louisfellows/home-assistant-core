"""Test the Combined Media Player config flow."""
...
async def test_config_flow(hass: HomeAssistant, platform) -> None:
    ...
    with patch(
        "homeassistant.components.combined_media.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"name": "My combined_media", "entity_id": input_sensor_entity_id},
        )
        await hass.async_block_till_done()

    assert result["type"] == FlowResultType.CREATE_ENTRY
    assert result["title"] == "My combined_media"
    assert result["data"] == {}
    ...
    assert config_entry.title == "My combined_media"
...
async def test_options(hass: HomeAssistant, platform) -> None:
    ...
    state = hass.states.get(f"{platform}.my_combined_media")
